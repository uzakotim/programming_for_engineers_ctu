...
class BinaryTree:
    ...
    def sumKeysDepth(self,node,curDepth,givenDepth):
        if node == None: return 0
        if curDepth == givenDepth:
            return node.key     
        total = self.sumKeysDepth(node.left, curDepth+1,givenDepth) + self.sumKeysDepth(node.right, curDepth+1,givenDepth)
        return total

    def sumLeaves(self,node):
        if node == None: return 0
        if node.left == None and node.right == None: return node.key
        return self.sumLeaves(node.left) + self.sumLeaves(node.right)
...
print( "The sum of keys of nodes at depth", t.sumKeysDepth(t.root,0,2) )
